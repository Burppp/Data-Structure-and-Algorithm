...
int length_helper(TreeNode* root, int &sum)
{
	if (!root->left && !root->right)
	{
		sum += root->val * (root->deep - 1);
		return sum;
	}
	length_helper(root->left, sum);
	length_helper(root->right, sum);
	return sum;
}
void treeLength(TreeNode* root)
{
	int sum = 0;
	length_helper(root, sum);
	cout << sum << endl;
}
vector<int>arr;
vector<int>arr_orin;
