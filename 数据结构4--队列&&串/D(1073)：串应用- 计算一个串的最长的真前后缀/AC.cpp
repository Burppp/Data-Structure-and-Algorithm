...
{
    int i, count = 0;
    string* ch1 = new string[this->mainstr.size()];
    string* ch2 = new string[this->mainstr.size()];
    for (i = 1; i < this->mainstr.size(); i++, count++)
    {
        ch1[count] = this->mainstr.substr(0, i);
        ch2[count] = this->mainstr.substr(this->mainstr.size() - i, i);
    }
    int max = -1;
    for (i = 0; i < count; i++)
        if (ch1[i] == ch2[i])
